...
class CropPlanModel:
    ...
    """
    def __init__(self):
        # We use Mistral:latest (Local) as the primary brain for the final Crop Plan generation.
        self.ollama = OllamaClient(model="mistral:latest")
        self.kb = VectorDB()
...
{{
  ...
  "pesticide_plan": {{ "preventative": "...", "curative": "..." }}
        ...
        try:
            raw = await self.ollama.get_completion(messages, temperature=0.2)
            start = raw.find('{')
            end = raw.rfind('}') + 1
