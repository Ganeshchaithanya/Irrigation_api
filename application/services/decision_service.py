...
from application.services import irrigation_service, state_service
...
class DecisionService:
    ...
    async def _run_ai_pipeline(self, db: Session, device_id):
        ...
        try:
            ...
            # 3. Collect last 24 soil moisture readings for the device (NodeData)
            # Use device.device_uid because NodeData stores the hardware string
            readings = (
                db.query(NodeData)
                .filter(NodeData.device_id == device.device_uid)
            ...
            # 4. Fetch the latest Zone-level environment (ZoneData)
            # Use str(zone_id) because ZoneData stores zone_id as a string for loose coupling
            zone_env = (
                db.query(ZoneData)
            ...
            # 5. Fetch the latest Master-level system metrics (MasterData)
            master_env = (
                db.query(MasterData)
            ...
            for i, r in enumerate(reversed(readings)):  # oldest first
                entry = {
                    ...
                    # Fallback to current zone context for all historical points (to keep AI input consistent)
                    "temp": (zone_env.temperature if zone_env else 25.0),
                    "humidity": (zone_env.humidity if zone_env else 60.0),
                    "flow": (master_env.flow_rate if master_env else 0.0),
                    "water_consumed": (master_env.water_consumed if master_env else 0.0),
                }
            ...
            # 6. Get Neighbors in the same Zone
            neighbors = []
    ...
    async def _process_ai_report(self, db: Session, zone_id, report: dict, latest_reading: dict):
        ...
        user_id = None
        try:
            from application.models.farm import Acre, Farm
        ...
        for anomaly in anomalies:
            ...
            # Lookup device for more specific context
            device_id_str = anomaly.get("device_id")
            device_label = "System"
            try:
                device = db.query(Device).filter(Device.device_uid == device_id_str).first()
                if device:
                    if device.role == "MASTER":
                        device_label = "MASTER GATEWAY"
                    else:
                        device_label = f"{device.position_label} NODE"
            except Exception:
                pass
            ...
            notif_svc.create(
                ...
                title=f"⚠️ [{device_label}] {anomaly.get('type', 'FAULT')}",
                message=anomaly.get("ai_diagnosis", "Hardware anomaly detected. Please inspect.")
            )
        ...
        if backend_action != "IDLE" and "VALVE_ON" in backend_action:
            ...
            if not existing:
                ...
                from application.models.ai import Prediction
        ...
        if validation:
            from application.models.ai import AiValidationLog
            log = AiValidationLog(
                zone_id=zone_id,
