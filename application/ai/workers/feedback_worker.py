...
class FeedbackWorker:
    ...
    def _calibrate_record(self, db: Session, record: IrrigationFeedback):
        ...
        peak_reading = (
            ...
            .filter(NodeData.timestamp >= record.irrigation_ended_at)
            .order_by(NodeData.timestamp.asc()) # Get the first one after end
            .first()
        )
        ...
        final_reading = (
            db.query(NodeData)
            .filter(NodeData.device_id.in_(device_ids))
            .filter(NodeData.timestamp <= record.feedback_recorded_at)
