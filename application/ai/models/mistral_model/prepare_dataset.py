...
def prepare_dataset():
    # Centralized seasonal data is now in application/data/
    data_base = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
    data_dir = os.path.join(data_base, "data")
    files = ["kharif.json", "rabi.json", "zaid.json"]
    
    # Save the training data output back to a data subdirectory in the models folder
    output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), "train_data.jsonl")
