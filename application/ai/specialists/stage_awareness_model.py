...
class StageAwarenessModel:
    ...
    def get_stage_context(
        self, crop: str, day: int, season: str = "Kharif"
    ) -> StageContext:
        """
        Main lookup. Returns a structured StageContext dict.
        Includes a cross-season fallback if the crop is not found in the target season.
        """
        # 1. Try primary season
        guide = self._load_guide(season)
        crop_data = self._find_crop(guide, crop)

        # 2. Cross-season fallback (e.g. if Tomato is in Kharif but we are in Zaid)
        if not crop_data:
            alt_seasons = [s for s in ["Kharif", "Rabi", "Zaid"] if s != season]
            for alt in alt_seasons:
                alt_guide = self._load_guide(alt)
                crop_data = self._find_crop(alt_guide, crop)
                if crop_data:
                    print(f"   [STAGE] '{crop}' found in alternate season guide: {alt}")
                    break

        if not crop_data:
            print(f"   [STAGE] '{crop}' not found in any guides. Falling back to default.")
            return self._default_context(crop, day)

        farming_guide = crop_data.get("complete_farming_guide", {})
        ...
        print(f"[STAGE] Day {day} out of range for crop '{crop}'.")
        return self._default_context(crop, day)
